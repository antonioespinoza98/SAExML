...
```{r}
y %>% ggplot(aes(x = ingreso)) + geom_density(stat = "density", fill = "red", alpha = 0.5) + theme_minimal() + xlim(0, 1000000)
```
...
ggplot(modelo$evaluation_log, aes(
  ...
  geom_vline(xintercept = 29, col = "red", linetype = 2) + theme_minimal()
```
...
xgb_train_rmse %>% ggplot(aes(x = simulacion, y = .)) + 
  geom_line() +
  geom_vline(xintercept = 1585, linetype = 2, col = "red") +
  # xlim(215, 300) +
  theme_minimal() 
```
...
ggplot(modelo$evaluation_log, aes(
  ...
  geom_vline(xintercept = 163, col = "red", linetype = 2) + theme_minimal()
```
...
pi1 <- xgb.ggplot.importance(importancia)
pi2 <- xgb.ggplot.importance(importancia2)
...
# Predicción censo con modelo reducido (parámetros)
