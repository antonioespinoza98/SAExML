...
xgb_train_rmse %>% ggplot(aes(x = simulacion, y = .)) + 
  geom_line() + 
  geom_vline(xintercept = 188, linetype = 2, col = "red") + 
  geom_vline(xintercept = 142, linetype = 2, col = "red")
...
ggplot(modelo$evaluation_log, aes(
  x = iter,
  y = train_rmse
)) + geom_line() + geom_vline(xintercept = 44, col = "red",linetype = 2)


modelo$evaluation_log[which.min(modelo$evaluation_log$train_rmse), ]

modelo$evaluation_log[43, ]
...
```{r}
matrix(c(mean(train$ingreso, mean(test$pred))),
