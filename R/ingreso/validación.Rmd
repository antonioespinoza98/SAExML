...
fit <- readRDS("ingreso/output/modelo_tree.rds")
...
sparse_matrix <- sparse.model.matrix( ~ ., data = poststrat_df)[, -1]
...
poststrat_df %<>% select(all_of(names_sel))
...
jpeg(file = "CRI/2021/1.Ingreso/Output/Plot_Bench_Pobreza.jpeg")
hist(poststrat_df$gk)
dev.off()

saveRDS(poststrat_df %>% select(!matches("_\\d{,2}$")), 
        "CRI/2021/1.Ingreso/Data/poststrat_df.RDS")
saveRDS(encuesta_mrp, "CRI/2021/1.Ingreso/Data/encuesta_mrp.RDS")
