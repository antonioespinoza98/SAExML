...
Para encuesta no tenemos la variable etnia, por lo que hacemos un left join de censo con encuesta para agregar esta variable de acuerdo a los distritos en la encuesta.
...
```{r}
mod = rpart(pobreza ~ ., method = "class",cp = 0.01, data = encuesta_mrp)
```
