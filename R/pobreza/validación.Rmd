...
```{r}
encuesta_mrp <- readRDS("pobreza/datos/encuesta_mrp1.rds") %>%
...
```{r}
confint_int <- confint(svyby(formula = ~pobreza,
                             by = ~region,
                             design = diseno,
                             svymean))
...
confint_int$pobreza <- media_est_srvyr$pobreza
...
```{r}
censo_mrp <- censo_mrp <- readRDS("pobreza/datos/censo_mrp1.rds") %>%
  dplyr::filter(anoest != "98") |>
      mutate(
  region = recode(dam,
               "01" = "Central",
...
```{r}
pred <- readRDS("pobreza/output/pred_censo.rds")
```

```{r}
censo_mrp$pred_pobreza <- pred
```

## Calculo pobreza

```{r}
pobreza_dam <- censo_mrp %>% 
  group_by(region) %>% 
  summarise(
    pobreza = mean(pred_pobreza)
  ) 
```


```{r}
mapping <- rbind(media_est_srvyr[,-3], pobreza_dam)
...
```{r}
p2 <- mapping %>%
  ggplot(aes(x = region, y = pobreza)) + geom_point(aes(color = estim), size = 2, position = "jitter") +
  scale_color_manual(values = c("red", "green")) +
  labs(title = "Validación para modelo de árbol", x = "Región de planificación", y = "Pobreza", col = "Modelo") +
...
```{r}
saveRDS(p2, "pobreza/output/validacion_arbol.rds")
```
